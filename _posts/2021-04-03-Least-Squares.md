...
$$\mathbf A$$는 $$m \times n$$의 Matrix이고, $$\mathbf{x}$$는 $$k \times 1$$의 Vector일 때, $$\mathbf{x}$$의 $$\mathbf A$$의 Column Space $$CS(\mathbf{A})$$에 대한 Projection Vector $$proj_{CS(\mathbf A)}\mathbf{x}$$는 다음과 같이 계산합니다.

$$\mathbf{y}$$는 $$n \times 1$$의 Vector이고 다음과 같이 정의합니다.

$$
proj_{CS(\mathbf A)}\mathbf{x}=\mathbf{A}\mathbf{y}
$$

$$
\mathbf{x}=proj_{CS(\mathbf A)}\mathbf{x}+proj_{CS(\mathbf A)^\perp}\mathbf{x} \\
\mathbf{x}-proj_{CS(\mathbf A)}\mathbf{x} \in CS(\mathbf{A})^\perp=Null(\mathbf{A}^T) \\
\mathbf{A}^T(\mathbf{x}-proj_{CS(\mathbf A)}\mathbf{x})=\mathbf{0} \\
\mathbf{A}^T(\mathbf{x}-\mathbf{A}\mathbf{y})=\mathbf{0} \\
\mathbf{A}^T\mathbf{x}-\mathbf{A}^T\mathbf{A}\mathbf{y}=\mathbf{0} \\
\mathbf{y}=(\mathbf{A}^T\mathbf{A})^{-1}\mathbf{A}^T\mathbf{x} \\
\begin{aligned}
proj_{CS(\mathbf A)}\mathbf{x}
&=\mathbf{A}\mathbf{y} \\
...
$$
proj_{CS(\mathbf A)}\mathbf{x}=\mathbf{A}(\mathbf{A}^T\mathbf{A})^{-1}\mathbf{A}^T\mathbf{x}
$$
...
$$
\arg\min_\mathbf{x} \left \| \mathbf{A}\mathbf{x}-\mathbf{b} \right \|_2^2 = (\mathbf{A}^T\mathbf{A})^{-1}\mathbf{A}^T\mathbf{b}
$$
...
$$
\arg\min_\mathbf{x}(\left \| \mathbf{A}\mathbf{x}-\mathbf{b} \right \|_2^2 + \lambda \left \| \mathbf{x} \right \|_2^2)=(\mathbf{A}^T\mathbf{A}+\lambda\mathbf{I})^{-1}\mathbf{A}^T\mathbf{b}
$$
...
$$
\arg\min_\mathbf{x}\left (\sum_{i=1}^k \lambda_i \left \| \mathbf{A}_i\mathbf{x}-\mathbf{b}_i \right \|_2^2 \right )=\left (\sum_{i=1}^k\lambda_i\mathbf{A}_i^T\mathbf{A}_i \right )^{-1} \left (\sum_{i=1}^k\lambda_i\mathbf{A}_i^T\mathbf{b}_i \right )
$$
...
\end{aligned} \\
\begin{aligned}
\arg\min_\mathbf{x}\left \| \sqrt{\mathbf{W}}\mathbf{A}\mathbf{x}-\sqrt{\mathbf{W}}\mathbf{b} \right \|_2^2
...
$$
\arg\min_\mathbf{x}(\mathbf{A}\mathbf{x}-\mathbf{b})^T\mathbf{W}(\mathbf{A}\mathbf{x}-\mathbf{b})=(\mathbf{A}^T\mathbf{W}\mathbf{A})^{-1}\mathbf{A}^T\mathbf{W}\mathbf{b}
$$
...
\end{aligned} \\
\begin{aligned}
\arg\min_\mathbf{x}\left \| \mathbf{L}^{-1}\mathbf{A}\mathbf{x}-\mathbf{L}^{-1}\mathbf{b} \right \|_2^2
...
$$
\arg\min_\mathbf{x}(\mathbf{A}\mathbf{x}-\mathbf{b})^T\mathbf{C}^{-1}(\mathbf{A}\mathbf{x}-\mathbf{b})
=(\mathbf{A}^T\mathbf{C}^{-1}\mathbf{A})^{-1}\mathbf{A}^T\mathbf{C}^{-1}\mathbf{b}
$$

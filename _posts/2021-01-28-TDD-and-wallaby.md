...
또한 `테스트 수행이 간편해야 합니다.` TDD는 코드를 작성하고 테스트를 돌리고, 다시 리팩토링을 한뒤 다시 실패하는 케이스를 넣고 코드를 넣는 등 짧은 호흡에 여러 번의 테스트를 수행합니다. 이러다보니 테스트를 돌리는 데 힘이 들면 TDD를 금방 포기해버리게 됩니다. ( 코드 한 줄 작성하고 `npm test`를 매번 돌릴다고 생각해보시면 한숨이 나올수 있어요.)
...
![message](/techblog/assets/images/TDD-and-Wallaby/message1.jpg)
...
![slack](/techblog/assets/images/TDD-and-Wallaby/slack.jpg)
...
![code](/techblog/assets/images/TDD-and-Wallaby/code.png)
...
- 간편한 테스트 실행
  - 위에도 적었지만, TDD를 위해서는 테스트 수행이 간편해야 합니다. 테스트를 수행할 때마다 `npm test`를 수행하기에는 너무 번거롭고 시간 소요가 드는 작업입니다.
  - 또한 `npm test`를 수행하게 되면 전체 테스트가 다 동작해서 시간도 오래 걸립니다. 아니면 `grunt`나 `gulp test script`를 수정해줘야 하는데 개발을 할 때마다 수정하는 것도 일입니다.
  - `wallaby.js`는 이렇게 테스트를 수행하는데 걸리는 시간을 단축시켜 줍니다. `repository root`에 `wallaby.js` 파일 하나만 추가해주면 됩니다.
...
[wallabyjs](https://wallabyjs.com/)에 접속해보시면 동영상으로 사용 예제를 보실수 있습니다. 

예제 동영상을 보시고 일단 커뮤니티 버전으로 일주일이라도 써보시는것을 추천드립니다.

## 마지막으로..

여기까지가 해당 포스트 내용의 끝입니다. 제가 TDD의 신봉자는 아니지만 TDD로 개발을 진행하며서 제가 작성한 코드의 자신감을 가지게 되었고, 버그를 줄이는데 많은 도움을 받았습니다. 

저처럼 버그 및 실수에 고통받으시는 개발자 분들은 TDD를 이용하여 개발을 진행하시는 것을 추천드립니다.
